				...
				<table id="datatable" class="table table-striped dt-responsive nowrap table-vertical" width="100%"
					...
					<tbody>
						...
						<tr>
							...
							<td><?php
        // Hitung total stok dari tbl_produk_variasi
        $id_produk = $produk['id_produk'];
        $qstok = mysqli_query($db, "SELECT SUM(stok) as total_stok FROM tbl_produk_variasi WHERE id_produk='$id_produk'");
        $stok = mysqli_fetch_assoc($qstok);
        echo number_format($stok['total_stok'] ?? 0);
    ?></td>
							<td>
